...
import { CreditCardIcon, Receipt, FileText, Download, CheckCircle, Calendar, Plus, FileDown, HelpCircle, Circle } from 'react-icons/all';
          ...
          <CardContent className="p-4">
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                <p className="text-2xl font-bold">{formatCurrency(225.00)}</p>
              </div>
              <CreditCardIcon className="h-10 w-10 text-amber-500 p-2 bg-amber-100 rounded-full" />
            </div>
            <Button className="w-full mt-2 bg-[#006D77] hover:bg-[#00585F]">Pay Now</Button>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                <p className="text-2xl font-bold">{formatCurrency(1375.50)}</p>
              </div>
              <Receipt className="h-10 w-10 text-purple-500 p-2 bg-purple-100 rounded-full" />
            </div>
            <div className="mt-2 text-sm text-gray-600">
              <div className="flex justify-between">
                <span>Insurance Covered:</span>
                <span className="font-medium">{formatCurrency(1100.00)}</span>
              </div>
              <div className="flex justify-between">
                <span>Out-of-Pocket:</span>
                <span className="font-medium">{formatCurrency(275.50)}</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                <p className="text-2xl font-bold">{formatCurrency(750)}/{formatCurrency(1500)}</p>
              </div>
              <FileText className="h-10 w-10 text-blue-500 p-2 bg-blue-100 rounded-full" />
            </div>
            <div className="mt-3">
              <Progress value={50} className="h-2" />
              <p className="text-sm text-gray-600 mt-1">50% of deductible met</p>
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              {filterItems(payments).length > 0 ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    {searchTerm || dateFilter ? 'Try adjusting your filters' : 'Payments will appear here once made'}
                  </p>
                </div>
              )}
              ...
              <div className="mt-8">
                ...
                {upcomingBills.length > 0 ? (
                    ...
                    {upcomingBills.map((bill) => (
                      <div key={bill.id} className="border rounded-lg p-4 hover:bg-gray-50">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-xs text-gray-500">
                              Insurance covers: {Math.round(bill.insuranceCoverage * 100)}%
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center text-sm mt-2">
                          ...
                          <span className="text-gray-600">Due: {new Date(bill.dueDate).toLocaleDateString()}</span>
                  ...
                  <div className="text-center py-8 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-gray-700 font-medium">No upcoming bills</p>
            ...
            <CardContent>
              {filterItems(claims).length > 0 ? (
                    ...
                    <tbody>
                      {filterItems(claims).map((claim) => (
                        <tr key={claim.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 font-medium">{formatCurrency(claim.amount)}</td>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    {searchTerm || dateFilter ? 'Try adjusting your filters' : 'Submit a claim to get started'}
                  </p>
                  <Button 
              ...
              <div className="mt-8 p-4 bg-blue-50 rounded-lg border border-blue-100">
                ...
                <ul className="space-y-2 text-sm text-blue-700">
                  <li className="flex items-start">
                    <Circle size={6} className="mr-2 mt-1.5" fill="currentColor" />
                    Submit claims within 90 days of service for faster processing
                  </li>
                  <li className="flex items-start">
                    <Circle size={6} className="mr-2 mt-1.5" fill="currentColor" />
                    Include all relevant documentation like receipts and referrals
                  </li>
                  <li className="flex items-start">
                    <Circle size={6} className="mr-2 mt-1.5" fill="currentColor" />
                    ...
                    <div>
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">{plan.groupNumber}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-600">Coverage Start:</span>
                          <span className="font-medium">{new Date(plan.coverageStart).toLocaleDateString()}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        <div className="flex justify-between mb-1">
                          ...
                          <span className="text-sm font-medium">
                            {formatCurrency(plan.deductibleMet)} / {formatCurrency(plan.deductible)}
                          </span>
                        </div>
                        <Progress 
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          {Math.round((plan.deductibleMet / plan.deductible) * 100)}% of deductible met
                        </p>
                      </div>
                      ...
                      <div>
                        <div className="flex justify-between mb-1">
                          ...
                          <span className="text-sm font-medium">
                            {formatCurrency(plan.outOfPocketMet)} / {formatCurrency(plan.outOfPocketMax)}
                          </span>
                        </div>
                        <Progress 
                          value={(plan.outOfPocketMet / plan.outOfPocketMax) * 100} 
                          className="h-2" 
                        />
                        <p className="text-xs text-gray-500 mt-1">
                          {Math.round((plan.outOfPocketMet / plan.outOfPocketMax) * 100)}% of maximum met
                        </p>
                      </div>
                    </div>
                  </div>
